...
if (!defined('PG_VIEW_TYPE')) {
    define('PG_VIEW_TYPE', 'file');
}
if (!defined('PG_POST_FILTER')) {
    define('PG_POST_FILTER', true);
}
if (!defined('PG_DEBUG')) {
    define('PG_DEBUG', false);
}
if (!defined('PG_AUTO_DISPLAY')) {
    define('PG_AUTO_DISPLAY', true);
}
if (!defined('PG_ROUTE')) {
    define('PG_ROUTE', false);
}
if (!defined('PG_CLOSE_CACHE')) {
    define('PG_CLOSE_CACHE', false);
}
...
if (!defined('PG_SESSION_TYPE')) {
    define('PG_SESSION_TYPE', 'file');
}
//session start
if (!defined('PG_SESSION_START')) {
    define('PG_SESSION_START', false);
}
//session host [memcache 11211 redis 6379]
if (!defined('PG_SESSION_HOST')) {
    define('PG_SESSION_HOST', 'tcp://127.0.0.1:11211');
}
...
// 页面后缀
if (!defined('PG_SUFFIX')) {
    define('PG_SUFFIX', false);
}
...
class pgException extends Exception {
    ...
    public function showBug() {
        if (PG_DEBUG) {
            include PG_IN . 'templates' . PG_DS . 'debug.php';
            pgExit();
        }
    }
...
function tool($args) {
    ...
    if (empty($staticTools[$className])) {
        ...
        if (!is_file($fileUri)) {
            throw new pgException("类文件 {$toolName} 不存在");
        }
        include $fileUri;
...
class grace {
    ...
    public function __init() {
        ...
        if (!empty($this->gets)) {
            $this->gets = str_replace(array('<', '>', '"', "'"), array('&lt;', '&gt;', '&quot;', ''), $this->gets);
        }
    }
    ...
    protected function getCacher() {
        ...
        $config = sc('cache');
        if (empty($config)) {
            throw new pgException('缓存设置错误');
        }
        if (!in_array($config['type'], sc('allowCacheType'))) {
            throw new pgException('缓存类型错误');
        }
...
function startSession() {
    switch (PG_SESSION_TYPE) {
        case 'file' :
            if (!is_dir(PG_SESSION_DIR)) {
                mkdir(PG_SESSION_DIR, 0777, true);
            }
...
function c($key1, $key2 = null) {
    static $config = null;
    if ($config == null) {
        $config = require PG_PATH . '/config.php';
    }
    if (is_null($key1)) {
        return $config;
    }
...
try {
    ...
    if (!$controller instanceof grace) {
        throw new pgException('[ ' . $controllerName . ' ] 必须继承自 grace');
    }
    $methodName = $router[1];
